...
- **Contributions** (`ContributionService`, see `contribution_design.md`): a viewer (member or public visitor) proposes a new question; editors review it across 1-n rounds (`approve`/`reject`/`request_changes`); once approved the **contributor merges**, which creates the question. Floors: `SubmitContribution`/`UpdateContribution`/`ListMyContributions`/`WithdrawContribution`/`MergeContribution` → `viewer`; `ListContributions`/`ReviewContribution` → `editor`. A contribution also carries a free-form **comment** thread (`contribution_comments`, distinct from the decision-bearing reviews); `AddContributionComment` → `viewer` (any member/visitor may join the discussion — the service only binds the comment to the bank). **Merge is the deliberate exception** to "content creation is editor-only": it is viewer-floor (the contributor may be a public visitor) but creates a `Question` — safe because the service gates it on `status == approved`, which only an editor's review can produce, and `UpdateContribution` resets `approved → pending` to dismiss a stale approval. The role floor authorizes *who acts on the bank*; the `status==approved` gate authorizes *what gets created*.
