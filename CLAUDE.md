...
- **Contributions** (`ContributionService`, see `contribution_design.md`): a viewer (member or public visitor) proposes a new question; editors review it across 1-n rounds (`approve`/`reject`/`request_changes`); once approved the **contributor merges**, which creates the question. Floors: `SubmitContribution`/`UpdateContribution`/`ListMyContributions`/`WithdrawContribution`/`MergeContribution` → `viewer`; `ListContributions`/`ReviewContribution` → `editor`. **Merge is the deliberate exception** to "content creation is editor-only": it is viewer-floor (the contributor may be a public visitor) but creates a `Question` — safe because the service gates it on `status == approved`, which only an editor's review can produce, and `UpdateContribution` resets `approved → pending` to dismiss a stale approval. The role floor authorizes *who acts on the bank*; the `status==approved` gate authorizes *what gets created*.
...
- Unit of Work pattern for all multi-table operations. The UoW is **closure-based**: `uow.Do(ctx, func(tx *uow.Store) error { … })` runs the closure in one transaction and commits on `nil`, rolls back on error or panic. `tx *uow.Store` exposes every repository bound to that tx. Keep non-transactional side effects (cache writes, the post-commit re-read) **after** `Do` returns — anything inside the closure is rolled back on failure. Do NOT hand-manage `Begin/Commit/Rollback`.
