...
export const passagesApi = {
  ...
  create: (bankId: string, data: { category_id: number; title: string; body: string; difficulty: string }) =>
    client.post<Passage>(`/banks/${bankId}/passages`, data).then(r => r.data),
  update: (bankId: string, id: string, data: Partial<{ category_id: number; title: string; body: string; difficulty: string }>) =>
