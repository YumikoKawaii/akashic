...
export function useCreatePassage(bankId: string) {
  ...
  return useMutation({
    mutationFn: (data: { category_id: string; title: string; body: string; difficulty: string }) =>
...
export function useUpdatePassage(bankId: string) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<{ category_id: string; title: string; body: string; difficulty: string }> }) =>
...
export function useDeletePassage(bankId: string) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => passagesApi.delete(bankId, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: passageKeys.all(bankId) }),
  })
}
